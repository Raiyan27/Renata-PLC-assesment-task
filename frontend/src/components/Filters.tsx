...
export default function FilterBar({ filters, onChange, categories }: Props) {
  ...
  const toggleAllReasons = () => {
    if (allSelected || selectedReasons.length === 0) {
      // If all selected or none selected (which means 'All' implicitly), 
      // clicking it clears the filter or sets to all.
      // Usually "Select All" checked state means everything is selected.
      if (selectedReasons.length === 0) {
        // If none selected, we select all to be explicit
        onChange({ ...filters, reason: [...categories] });
      } else {
        // If all selected, we clear it (which also means All Categories in our backend)
    ...
    <div className="flex flex-wrap items-center gap-4 p-4 px-5 bg-white border border-slate-200 rounded-xl mb-6 shadow-sm relative z-20">
      ...
      <div className="flex flex-col gap-1 ml-2 pl-4 border-l border-slate-200">
        <label
          ...
          className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer pt-6"
        >
          <input
            id="filter-valid"
            type="checkbox"
      ...
      <div className="flex-1 flex justify-end items-center h-full mt-auto pt-6">
        <button
